...
class DogFightEnv(gym.Env):
    ...
    def _build_observation_for(
        ...
        *,
        ...
        return build_observation(
            ...
            bt_action=bt_action,
            health_source=str(
                getattr(self, "config", {}).get("observation_contract", {}).get(
    ...
    def _get_phase_damage_coeff(self, dis_m: float, ata_abs_deg: float) -> float:
        # Phase 1: <= 3000 ft and <= 1.0 deg (angle_deg=2.0 -> half=1.0)
        if dis_m <= 3000.0 * 0.3048 and ata_abs_deg <= 1.0:
            return 1.0
        # Phase 2: <= 3500 ft and <= 2.0 deg (angle_deg=4.0 -> half=2.0)
        elif dis_m <= 3500.0 * 0.3048 and ata_abs_deg <= 2.0:
            return 0.3
        # Phase 3: <= 4000 ft and <= 3.0 deg (angle_deg=6.0 -> half=3.0)
        elif dis_m <= 4000.0 * 0.3048 and ata_abs_deg <= 3.0:
            return 0.1
        return 0.0
    ...
    def update_damage(self):
        ...
        if base_range_m != 0 and min_range_m <= dis_m <= max_range_m:
            if half_wez_angle_deg >= abs(ownship_ata_deg):
                coeff = self._get_phase_damage_coeff(dis_m, abs(ownship_ata_deg))
                target_damage = ((max_range_m - dis_m) / base_range_m) * self._delta_t * coeff
            if half_wez_angle_deg >= abs(target_ata_deg):
                coeff = self._get_phase_damage_coeff(dis_m, abs(target_ata_deg))
                ownship_damage = ((max_range_m - dis_m) / base_range_m) * self._delta_t * coeff
    ...
    def _write_log(self, path: str, entries: List[List[float]]) -> None:
        with open(path, "w", encoding="utf-8") as file:
            ...
            for step, item in enumerate(entries):
                time_value = np.floor(self._delta_t * self._step_ratio * step * 10000) / 10000
