...
class HybridActionProvider(ActionProvider):
    ...
    def compute_action(self, context: ActionContext) -> ActionResult:
        ...
        if self.mode == "blend":
            ...
            # Residual addition is appropriate for signed control surfaces, but
            # adding two non-negative throttle commands makes full-throttle
            # saturation almost unavoidable. Keep residual steering authority
            # while using a convex throttle blend.
            action = np.asarray(secondary_result.action, dtype=np.float32).copy()
            action[:3] += self.residual_scale * np.asarray(
                primary_result.action[:3], dtype=np.float32
            )
            throttle_weight = float(np.clip(self.residual_scale, 0.0, 1.0))
            action[3] = (
                (1.0 - throttle_weight) * float(secondary_result.action[3])
                + throttle_weight * float(primary_result.action[3])
            )
