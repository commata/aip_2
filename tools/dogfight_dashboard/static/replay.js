...
function updateHud(ownIndex, targetIndex) {
  ...
  const ownAa = angleBetweenDeg(targetForward, relative);
  ...
  $("hud-left").textContent =
    `${state}  t=${State.simTime.toFixed(2)}s  x${State.speed.toFixed(1)}\n` +
    `Own  alt=${fmt0(ownPos[2] * 3.28084)}ft  v=${fmt1(ownSpeed * 1.94384)}kts  hp=${fmtHealth(own.health[ownIndex])}\n` +
    `Tgt  alt=${fmt0(targetPos[2] * 3.28084)}ft  v=${fmt1(targetSpeed * 1.94384)}kts  hp=${fmtHealth(target.health[targetIndex])}`;
